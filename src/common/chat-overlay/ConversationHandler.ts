...
import { getPersonaIdFromPurposeId, isCustomPersonaPurposeId } from '~/common/stores/chat/chat.conversation';
...
export class ConversationHandler {
  ...
  static inlineUpdatePurposeInHistory(conversationId: DConversationId, history: DMessage[], assistantLlmId: DLLMId | undefined): void {
    ...
    let systemMessage: DMessage = systemMessageIndex >= 0 ? history.splice(systemMessageIndex, 1)[0] : createDMessageEmpty('system'); // [chat] new system:'' (non updated)
    ...
    // - Built-in personas: resolved from the system-persona catalog (templates may be mixed via {{...}}).
    // - User-created personas (`persona:<uuid>`): resolved from the in-memory persona cache; we freeze
    //   the system message after first use (like 'Custom') so subsequent edits in Persona Manager don't
    //   silently rewrite an in-progress conversation's instructions.
    let resolvedSystemMessageText: string | null = null;
    if (purposeId) {
      if (isCustomPersonaPurposeId(purposeId)) {
        const personaId = getPersonaIdFromPurposeId(purposeId);
        const cached = personaId ? usePersonaCacheStore.getState().getPersona(personaId) : undefined;
        if (cached?.systemPrompt) resolvedSystemMessageText = bareBonesPromptMixer(cached.systemPrompt, assistantLlmId);
      } else {
        const systemPurpose = getSystemPurpose(purposeId);
        if (systemPurpose?.systemMessage) resolvedSystemMessageText = bareBonesPromptMixer(systemPurpose.systemMessage, assistantLlmId);
      }
    }

    if (!systemMessage.updated && purposeId && resolvedSystemMessageText) {
      systemMessage.purposeId = purposeId;
      systemMessage.fragments = [createTextContentFragment(resolvedSystemMessageText)];

      // HACK: pin the system message for 'Custom' and user-created personas so it isn't rewritten on persona switch / persona edit
      if (purposeId === 'Custom' || isCustomPersonaPurposeId(purposeId)) systemMessage.updated = Date.now();
  ...
  static inlineUpdateAutoPromptCaching(history: DMessage[]): void {
    ...
    for (let i = history.length - 1; i >= 0; i--) {
      ...
      if (!isSystemInstruction && history[i].role !== 'user') continue;
      ...
      if (autoState && messageHasUserFlag(history[i], MESSAGE_FLAG_VND_ANT_CACHE_USER)) autoState = false;
  ...
  messageAppendAssistantPlaceholder(
    placeholderText: string,
    update?: Partial<DMessage>,
  ): { assistantMessageId: DMessageId; placeholderFragmentId: DMessageFragmentId } {
    const message = createDMessagePlaceholderIncomplete('assistant', placeholderText);
    if (update) Object.assign(message, update);
    this.messageAppend(message);
  ...
  messagesDelete(messageIds: DMessageId[]): void {
    for (const messageId of messageIds) _chatStoreActions.deleteMessage(this.conversationId, messageId);
  ...
  historyReplace(messages: DMessage[]): void {
    ...
    if (!messages.length) this.beamStore.getState().terminateKeepingSettings();
  }
  ...
  historyViewHeadOrThrow(scope: string): Readonly<DMessage[]> {
    ...
    if (messages === undefined) throw new Error(`allMessages: Conversation not found, ${scope}`);
    return messages;
  }
  ...
  beamInvoke(viewHistory: Readonly<DMessage[]>, importMessages: DMessage[], destReplaceMessageId: DMessage['id'] | null): void {
    ...
    const onBeamSuccess = (messageUpdate: Pick<DMessage, 'fragments' | 'generator'>) => {
      ...
      if (chatThinkingPolicy === 'last-only') this.historyStripThinking(1);
      else if (chatThinkingPolicy === 'discard-all') this.historyStripThinking(0);

      // close beam
      terminateKeepingSettings();

      // auto-title the conversation if enabled (parity with chat-persona flow — fixes #1078)
      if (getChatAutoAI().autoTitleChat) void autoConversationTitle(this.conversationId, false);
    };
  ...
  createEphemeralHandler(title: string, initialText: string) {
    ...
    const deleteIfMinimized = () => {
      if (getEphemeral(eId)?.minimized) ephemeralsDelete(eId);
    };
