                  ...
                  items.forEach(item => {
                    try {
                      ...
                      request.onerror = (event) => {
                        ...
                        logger.error(`Error adding item to "${storeName}" in "${dbName}" (Key: ${typeof item.key === 'object' ? JSON.stringify(item.key) : item.key
...
export function FlashRestore(props: { unlockRestore?: boolean }) {
  ...
  const handleRestoreLoad = React.useCallback(async () => {
    ...
    try {
      ...
      if (data.metadata.application !== 'AI Tutor') {
