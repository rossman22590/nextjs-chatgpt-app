 ...
 * localai, minimax, mistral, modular, moonshot, nvidianim, openai, openrouter, perplexity, sakanaai,
 * togetherai, xai, zai
 */
...
export const openAIAccessSchema = z.object({
  dialect: z.enum([
    ...
    'localai', 'minimax', 'mistral', 'modular', 'moonshot', 'nvidianim', 'openai',
...
export function openAIAccess(access: OpenAIAccessSchema, modelRefId: string | null, apiPath: string): { headers: HeadersInit, url: string } {
  switch (access.dialect) {
    ...
    case 'minimax':
      ...
      let minimaxHost = llmsFixupHost(access.oaiHost || env.MINIMAX_API_HOST || DEFAULT_MINIMAX_HOST, apiPath);

      // tolerate hosts copied with the '/v1' suffix (MiniMax docs show the base URL that way) - apiPath already includes /v1
      if (minimaxHost.endsWith('/v1'))
        minimaxHost = minimaxHost.slice(0, -3);
